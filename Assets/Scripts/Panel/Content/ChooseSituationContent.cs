...
public class ChooseSituationContent : MonoBehaviour {
	...
	internal void Open(List<Situation> situations, int hour, GameObject panelWindow, Situation actualSituation) {
		...
		foreach(Situation s in situations) { 
			if (s.Selectable) {
				AddButton(s, actualSituation, panelWindow, hour);
			}
		}

		//add empty situation
		AddButton(new Situation(null, "", new List<Change>(), true, new List<LOL.Button>(), sNow), actualSituation, panelWindow, hour);
	}
